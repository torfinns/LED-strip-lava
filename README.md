...
| R2.1-2 | DEMO now fills one module over `DEMO_FILL_DURATION_SEC` (1.5 s) — same fill mechanic as normal ignition (organic per-column ramp via `useFillMask`), just much faster — instead of jumping straight to full flame. Combined with the ~1 s/hop cascade (R2.1-1), this smooths the wave down the chain instead of each module snapping stepwise from dark to full purple | Fix applied, needs a full 4-module hardware test |
