...
void beginStartPulseOut() {
  if (ttlStartPulseTriggered) return;
  ttlStartPulseTriggered = true;
  ttlStartPulseActive    = true;
  ttlStartPulseStartMs   = millis();
...
void setup() {
  ...
  digitalWrite(TTL_OUT_PIN, HIGH);  // hvile HIGH for aktiv LOW-protokoll

  pinMode(TTL_IN_PIN, INPUT_PULLUP);
...
void loop() {
  ...
  if (ttlMeasuredActive) {
    uint32_t widthNow = now - ttlActiveStartMs;
  ...
  // -----------------------------------------------
  // TTL OUT: fullfør aktive pulser uansett mode
  // -----------------------------------------------
  ...
  // -----------------------------------------------
  // TTL OUT: PLAYING-logikk
  // -----------------------------------------------
  if (mode == PLAYING) {
    if (!ttlStartPulseTriggered && t >= TTL_START_PULSE_SEC) {
      beginStartPulseOut();
    }
  ...
  for (uint8_t ring = 0; ring < RINGS; ring++) {
    ...
    for (uint8_t col = 0; col < ledsInRing; col++) {
      ...
      if (glowBlend >= 1.0f) {
        ...
        uint8_t fr = (flameColor >> 16) & 0xFF;
        uint8_t fg = (flameColor >> 8)  & 0xFF;
        uint8_t fb =  flameColor        & 0xFF;
